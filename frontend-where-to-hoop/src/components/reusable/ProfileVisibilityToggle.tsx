    ...
    <div className="flex flex-col gap-1">
      <label className="flex items-center justify-between gap-4">
        ...
        <div
          ...
          className={`relative shrink-0 w-12 h-6 rounded-full transition-colors duration-200 ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'} ${isChecked ? `${colorModeContext} bg-gray-600 dark:bg-gray-300` : 'bg-first-color'}`}
        >
          <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-background rounded-full shadow transition-transform duration-200 ${isChecked ? 'translate-x-6' : 'translate-x-0'}`} />
        </div>
      </label>
