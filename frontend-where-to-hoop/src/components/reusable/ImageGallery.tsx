...
import { minScreenSize } from '../../utils/constants'
...
export const ImageGallery = ({ images, name }: ImageGalleryProps) => {
  ...
  const {
    ...
    goTo,
    handlers,
  } = useSwipeDrag({ itemCount: images.length, currentIndex: selectedIndex, onIndexChange: setSelectedIndex })

  // Click to open lightbox (only if not dragging)
  const handleImageClick = () => {
    if (hasDragged() || images.length === 0) return
  ...
  const showRightFade = selectedIndex < images.length - 1 || dragOffset > 0
