...
const FollowingPlayerCard = ({ profile }: FollowingPlayerCardProps) => {
  ...
  return (
    <Link to={`/players/${profile.nickname.toLowerCase()}`}>
      <div className={`${colorModeContext} group flex items-center gap-3 p-3 rounded-lg bg-white/20 border border-black/30 hover:border-black/60 transition-colors dark:bg-black/20 dark:border-white/30 dark:hover:border-white/60`}>
